...
def plot_optimization_results(opt, mode, target_stress, confidence, top_n):
    ...
    # Only 4 panels: stress vs power, stress vs speed, stress vs VED, top-N bar
    ...
    # Panel 3: Stress vs VED
    ax3 = fig.add_subplot(gs[1, 0])
    ax3.scatter([c['VED']    for c in candidates],
                [c['Stress'] for c in candidates],
                c='steelblue', s=20, alpha=0.5, edgecolors='none')
    ...
    ax3.set_title('Stress vs VED', fontweight='bold')
    ax3.legend(fontsize=8); ax3.grid(alpha=0.3)

    # Panel 4: Top-N bar chart
    ax4 = fig.add_subplot(gs[1, 1])
    labels_b = [f'#{i+1}  P={c["Power_W"]:.0f}  v={c["Speed_mms"]:.0f}  h={c["Hatch_mm"]*1000:.0f}µm'
                for i, c in enumerate(top)]
    ax4.barh(range(len(top)), [c['Stress'] for c in top],
             xerr=[c['Std'] for c in top],
             color=['crimson'] + ['steelblue']*(len(top)-1),
             alpha=0.75, error_kw={'ecolor':'k','capsize':3,'linewidth':1})
    ax4.set_yticks(range(len(top))); ax4.set_yticklabels(labels_b, fontsize=7.5)
    ax4.invert_yaxis()
    ax4.axvline(best_stress, color='crimson', linestyle='--', linewidth=1, alpha=0.6)
    ax4.set_xlabel('Predicted Residual Stress')
    ax4.set_title(f'Top-{top_n} candidates  (red = best)', fontweight='bold')
...
def plot_prediction_result(power_W, speed_mms, hatch_mm,
    ...
    # Only 4 panels: VED prior, processing map, M3 vs M5 bar, confidence interval + hatch sweep
    fig = plt.figure(figsize=(16, 9))
    fig.suptitle(f'M5 Prediction  |  P={power_W:.0f}W  v={speed_mms:.0f}mm/s  h={hatch_mm*1000:.1f}µm',
                 fontsize=13, fontweight='bold')
    gs = fig.add_gridspec(2, 2, hspace=0.45, wspace=0.35)

    # Panel 1: VED prior curve + this point
    ax1 = fig.add_subplot(gs[0, 0])
    ...
    # Panel 2: Processing map with prediction marker
    ax2 = fig.add_subplot(gs[0, 1])
    ...
    # Panel 3: M3 vs M5 comparison bar
    ax3 = fig.add_subplot(gs[1, 0])
    ...
    ax4.plot(h_sweep*1000, s_arr, 'purple', linewidth=2)
    ax4.fill_between(h_sweep*1000, s_arr-sd_arr, s_arr+sd_arr,
                     alpha=0.2, color='purple', label='±1σ')
    ax4.axvline(hatch_mm*1000, color='red', linestyle='--', linewidth=1.5,
                label=f'Current h={hatch_mm*1000:.1f}µm')
    ax4.set_xlabel('Hatch Spacing (µm)'); ax4.set_ylabel('Predicted Stress')
    ax4.set_title('Stress vs Hatch  (P & v fixed)', fontweight='bold')
...
st.markdown(
    f'<p class="sub-header">Physics-Informed Bayesian ML — {len(df_data)} data points loaded • '
    f'Prior: {models_global["best_prior_name"].upper()} (R²={0.676:.3f})</p>',
    unsafe_allow_html=True
)
...
with tab1:
    ...
    if run_pred:
        ...
        # Metrics row — only stress predictions and VED/prior
        st.markdown("### Results")
...
with tab2:
    ...
    **Search bounds from your dataset:**
    ...
    if run_opt:
        ...
        # Best solution — no width/depth/WD metrics shown
        ...
        # Top-N table — exclude width, depth, WD columns
        ...
        with st.spinner("Generating optimization plots…"):
...
with tab3:
    ...
    # Show only user-relevant columns, hide internal ET melt pool columns
    display_cols = ['Power_W', 'Speed_mms', 'Hatch_mm', 'ED_Jmm3', 'Res_Stress']
    st.dataframe(df_data[display_cols].describe().round(3), use_container_width=True)
    st.markdown("### Sample Data")
    st.dataframe(df_data[display_cols].head(20), use_container_width=True)
